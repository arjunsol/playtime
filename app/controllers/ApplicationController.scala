...
object ApplicationController extends Controller {
    ...
    val applicationForm = Form(
        mapping(
            ...
            "parentId" -> optional(of[Long]))(ApplicationRow.apply)(ApplicationRow.unapply))
    ...
    def save(parentId: Long) = Action { implicit request =>
        applicationForm.bindFromRequest.fold(
            ...
            application => {
                ...
                val parentApp = ApplicationRow.findById(parentId)
                val path: String = parentApp match {
                    case Some(parentApp) => parentApp.path + "\\" + parentApp.name + "\\modules\\" + application.name
                    case None            => application.path
                }

                val applicationNew = new ApplicationRow(None, application.name, path, true, Some(parentId))

                val id = ApplicationRow.save(applicationNew)

                parentApp match {
                    case Some(parentApp) => { if (applicationNew.createProject) { ProjectOperations.createProject(applicationNew.name, applicationNew.path + "\\" + applicationNew.name) } }
                    case None            => { ProjectOperations.createModule(applicationNew.name, path, parentId) }
                }
